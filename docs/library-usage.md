...
通信器.邻机状态流.collect { 邻机列表 ->
    // 底层 BLE 诊断：显示逻辑连接通道、可写通道等
}

通信器.对手状态流.collect { 对手列表 ->
    // 业务 UI 建议看这里：只有存在 可发送=true 的对手时才允许业务出拳/发送
}
...
`邻机状态流` 是底层 BLE 通道状态，同一台手机可能因为服务端连接、客户端连接、
通知订阅等路径显示多条记录。业务 App 不建议直接用它判断“对手在线”。`对手状态流`
会把这些记录按稳定 ID 或可推断身份归并：

- `已发现`：扫描到对手，但还没有建立可用通信路径。
- `连接中`：已有连接，但通知订阅或写入通道还没有稳定完成。
- `可发送`：当前有可用发送路径，业务 UI 可以允许发送。
- `最近断开`：同一对手组里最新状态是断开，应回到寻找/等待状态。
...
- 当前实现聚焦最小示例，不做历史消息补发。
- 当前实现只报告“已写入”，不代表业务级送达确认；`notify=success` 和
  `write=success` 也只表示 Android 接受了本次请求。
