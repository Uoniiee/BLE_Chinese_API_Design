...
### 当前本地待测版本：v0.6.3-debug

更新时间：2026-06-02

本版本基于 BLE_RPS 剪刀石头布示例 `v1.0.30` 真机测试过程中暴露的
断开/重连问题，先做最小诊断 App，不直接并入业务 UI：
...
- 示例 App 改为最小诊断界面，日志包含 `[APP#]`、`[SEND#]`、
  `[PEERS]`、`[STATE]`，用于确认断开、重连、发送结果和陈旧通道清理。

`v0.6.1-debug` 仍保留为 Issue #1 已验收基线；`v0.6.2-debug` 是发送
链路保守修复版本；`v0.6.3-debug` 是加入结构化日志后的最小诊断待测版本。
