...
### 当前本地待测版本：v0.6.7-debug

更新时间：2026-06-02

本版本基于 `v0.6.6-debug` 增加一键保存诊断日志能力，方便真机测试后直接转发：

- 示例 App 新增“保存日记”按钮，会把当前日志保存为 UTF-8 文本文件。
- Android 10 及以上保存到系统 `Downloads` 目录；低版本保存到应用自己的下载目录。
- 保存文件会包含版本号、保存时间、设备型号、Android 版本和当前日志内容。
- 可见日志缓存从 260 行扩大到 1200 行，减少长流程测试时日志被截断的问题。

`v0.6.6-debug` 基于 `v0.6.5-debug` A/C 两机诊断日志继续补强停止/重启边界：

- 每次 `启动()` 会分配新的通信会话编号；`停止()` 会立刻让旧会话失效。
- 扫描、广播、GATT 服务端、GATT 客户端和写入回调都会核对会话编号，旧回调
  只记录“忽略过期回调”，不能再改邻机状态或把业务对手拉回 `ready=1`。
- 旧写入连接对象不能完成新连接的写入队列；停止后的后台写入会快速失败为
  “写入连接已过期”或“通信未启动”。

`v0.6.5-debug` 基于 `v0.6.4-debug` A/C 两机诊断日志继续修复断开/重连边界：

- 业务对手的“刚断开”状态会记录在归并后的对手层，不再因为某条随机地址
  记录老化而重新被旧 `w=true` 通道误判为 `可发送`。
- 当通知通道不可用且前台 client 写入失败时，会立刻移除失败写入路径并把
  对手降级为不可发送，避免反复出现“ready=1 但短测写入失败”。
- 保留 `subscribed=0` 但 client 写入成功的正常路径；也保留通知成功后的
  后台写入失败只记录日志、不立刻判定掉线。

`v0.6.4-debug` 基于 BLE_RPS 剪刀石头布示例 `v1.0.30` 真机测试过程中暴露的
断开/重连问题，把 `v0.6.3-debug` 的日志结论先收敛进中文 API：

- 新增 `对手状态流`、`对手状态` 与 `邻机就绪状态`，把同一台物理对手的
  扫描记录、服务端连接、客户端连接和通知订阅归并成业务可读状态。
- `发送()` 只对 `可发送` 的对手组放行；对手刚断开、正在重连、尚未完成通知
  订阅时，会返回失败而不是依赖陈旧通道制造“假成功”。
- 示例 App 继续保留最小诊断界面，新增 `[READY]` 日志和“对手就绪”显示；
  `[PEERS]` 仍用于观察底层 BLE 逻辑通道，不再作为业务 UI 的唯一依据。
- 当前 BLE 层的 `notify=success` / `write=success` 仍只表示 Android 接受了
  请求，不代表对端业务已经收到；业务级送达确认留作后续单独设计。

`v0.6.1-debug` 仍保留为 Issue #1 已验收基线；`v0.6.2-debug` 是发送
链路保守修复版本；`v0.6.3-debug` 是结构化日志诊断版本；`v0.6.4-debug`
是把诊断结论转成 API 对手就绪门控的待测版本；`v0.6.5-debug` 是远端断开
后防止旧物理通道误恢复的版本；`v0.6.6-debug` 是本机停止/重启时防旧回调
串台的待测版本；`v0.6.7-debug` 是增加一键保存日志的诊断版本。
